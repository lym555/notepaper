...
#endif /* CONFIG_COAP_MBEDTLS_PKI */
static void coap_weather_now(void *p)
{
    struct hostent *hp;
    coap_address_t    dst_addr;
    static coap_uri_t uri;
    const char       *server_uri = COAP_WEATHER_NOW_URL;
    char *phostname = NULL;
...
#define BUFSIZE 40
        ...
        phostname = (char *)calloc(1, uri.host.length + 1); //分配1个长度为参数2的内存空间
        if (phostname == NULL) {
            ESP_LOGE(TAG, "calloc failed");
            break;
        }

        memcpy(phostname, uri.host.s, uri.host.length);     //拷贝uri.host.s的uri.host.length个到phostname中
        ...
        request->code = COAP_REQUEST_GET;               //GET请求
        coap_add_optlist_pdu(request, &optlist);

        resp_wait = 1;                                  //等待超时
        coap_send(session, request);                    //发起请求
